...
from .models import Request, Payload, RequestQueue
from .models import Domain as DomainDB


def analyze_concurrency():
    print(__name__)
    requests_from_queue = RequestQueue.objects.all()
    requesties = list()

    for r in requests_from_queue:
        requesties.append(r.request)

    requesties = tuple(requesties)
    ...
    class Manager:
        ...
        def refresh_balance(self):
            keys_and_tokens = Payload.objects.all()
            for item in keys_and_tokens:
                r = requests.get(
                    f'https://checktrust.ru/app.php?r=host/app/summary/basic&applicationKey={item.key}&host=yandex.ru&parameterList=').json()
    ...
    class Yandex:
        ...
        def check_request_in_db(self):
            check = Request.objects.filter(request=self.request).first()
            if check:
                self.result = check
                return True
        ...
        def add_result_to_database(self):
            Request(request=self.request,
                    site_age_concurency= self.concurency_object.site_age_concurency,
                    site_stem_concurency= self.concurency_object.site_stem_concurency,
                    site_volume_concurency= self.concurency_object.site_volume_concurency,
                    site_backlinks_concurency= self.concurency_object.site_backlinks_concurency,
                    site_total_concurency= self.concurency_object.site_total_concurency,
                    direct_upscale= self.concurency_object.direct_upscale,
                    ).save()
    ...
    class Backlinks:
        ...
        def get_token(self):
            accounts_with_balance = Payload.objects.filter(balance__gte=30).first()
            self.token = accounts_with_balance.key
    ...
    class Domain:
        ...
        def check_data_in_database(self):
            check = DomainDB.objects.filter(name=self.domain).first()

            if check:
                self.domain_age = check.age
                self.backlinks = check.unique_backlinks
            else:
                self.get_domain_age()
                self.make_backlinks_object()
                self.backlinks = self.backlinks_object.unique_backlinks

                DomainDB(name=self.domain, age=self.domain_age, unique_backlinks=self.backlinks,
                         total_backlinks=self.backlinks_object.total_backlinks).save()

            if self.backlinks == 0:
                self.make_backlinks_object()
                self.backlinks = self.backlinks_object.unique_backlinks

                domainDB_object = DomainDB.objects.filter(name=self.domain).first()
                domainDB_object.unique_backlinks = self.backlinks_object.unique_backlinks
                domainDB_object.total_backlinks = self.backlinks_object.total_backlinks
        ...
        def calculate_site_stem_concurency(self):
            ...
            for site_object in self.site_objects_list:
                ...
                if site_object.site_type == 'organic':
                    ...
                    real_stem_concurency += int(matched_stem_items / len(self.request)) * self.WEIGHTS[
                        site_object.position]
                else:
            ...
            for site_object in self.site_objects_list:
                if site_object.site_type == 'direct':
                    if site_object.position == '1':
                        ...
                        direct_upscale = round(direct_upscale, 2)
                    logger.debug(
                        f'Директ обнаружен на позиции {site_object.position}, текущий upscale = {direct_upscale}')
